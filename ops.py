...
#     with tf.variable_scope(name):
#         W = tf.get_variable(name='weight', shape=kernel, initializer=tf.random_normal_initializer(mean=0.0, stddev=init_rate), collections=collection_name, trainable=trainable)
#         b = tf.get_variable(name='bias', shape=[kernel[-1]], initializer=tf.random_normal_initializer(mean=0.0, stddev=init_rate),collections=collection_name, trainable=trainable)
#         conv = tf.nn.conv2d(input, W*0.01 , strides=strides, padding=padding)
#         out = tf.nn.bias_add(conv, b*0.01 )
#         return out
