...
# def img_net(inputs, bit, numclass, reuse=False, name='Img_Network'):
...
#                 # current = batch_norm(current, name=str(name))
#             imgnet[name] = current
#         feature = {}
#         img_tmp_sqzd = []
#         img_tmp_usqzd = []
...
#             conv_name = 'img_feat_' + str(i)
#             bn_name = 'img_feat_BN_' + str(i)
#             feature[feat_name] = lrelu(batch_norm(
#                                 conv2d(input=current, kernel=[1, 1, 4096, SEMANTIC_EMBED], strides=[1, 1, 1, 1],
#                                        padding='VALID', init_rate=1.0, name=conv_name), name=bn_name))
#
#             img_tmp_sqzd.append(tf.squeeze(feature[feat_name], axis=1))
#             img_tmp_usqzd.append(feature[feat_name])
#
#         feature['sqzd_feature'] = tf.concat(img_tmp_sqzd, 1)
#
...
#             conv2d(input=imgnet['feature'], kernel=[1, 1, SEMANTIC_EMBED, bit], strides=[1, 1, 1, 1],
#                                        padding='VALID', init_rate=1.0, name='img_hash'), name='img_hash_BN'))
#
#         imgnet['label'] = sigmoid(batch_norm(
#             conv2d(input=imgnet['hash'], kernel=[1, 1, bit, numclass], strides=[1, 1, 1, 1],
#                                        padding='VALID', init_rate=1.0, name='img_label'), name='img_label_BN'))
#
#         # return tf.squeeze(imgnet['hash']), imgnet['feature'], tf.squeeze(imgnet['label'])
#         return tf.squeeze(imgnet['hash']), feature['sqzd_feature'], tf.squeeze(imgnet['label'])
#
# def txt_net(text_input, bit, dimy, numclass, reuse=False, name='Txt_Network'):
#     with tf.variable_scope(name):
#         if reuse:
#             tf.get_variable_scope().reuse_variables()
#         else:
...
#         LAYER1_NODE = 512
#             # MultiScal
#         # MultiScal = MultiScaleTxt(text_input, 'multiscale')
#         # txtnet['conv1'] = lrelu(batch_norm(
#         #                     conv2d(input=MultiScal, kernel=[1, dimy, 5, 4096], strides=[1, 1, 1, 1], padding='VALID',
#         #                                         init_rate=1.0, name='txt_ext_conv1'), name='txt_ext_conv1_BN1'))
#
#         txtnet['conv1'] = lrelu(batch_norm(
...
#         # txtnet['concat'] = tf.concat([txtnet['conv1'], txtnet['conv2']], axis=2, name="concat")
#
#         feature = {}
#         txt_tmp_sqzd = []
#         txt_tmp_usqzd = []
#         for i in range(N):
#             feat_name = 'feature_' + str(i)
#             conv_name = 'txt_feat_' + str(i)
#             bn_name = 'txt_feat_BN_' + str(i)
#             feature[feat_name] = lrelu(batch_norm(
#                                      # conv2d(input=txtnet['concat'], kernel=[1, 2, LAYER1_NODE, SEMANTIC_EMBED], strides=[1, 1, 1, 1], padding='VALID',
#                                      conv2d(input=txtnet['conv2'], kernel=[1, 1, LAYER1_NODE, SEMANTIC_EMBED], strides=[1, 1, 1, 1], padding='VALID',
#                                             init_rate=1.0, name=conv_name), name=bn_name))
#
#             txt_tmp_sqzd.append(tf.squeeze(feature[feat_name], axis=1))
#             txt_tmp_usqzd.append(feature[feat_name])
#
#         feature['sqzd_feature'] = tf.concat(txt_tmp_sqzd, 1)
#
#
...
#                             conv2d(input=txtnet['feature'], kernel=[1, 1, SEMANTIC_EMBED, bit], strides=[1, 1, 1, 1], padding='VALID',
#                                             init_rate=1.0, name='txt_hash'), name='txt_hash_BN'))
#
#         txtnet['label'] = sigmoid(batch_norm(
#                             conv2d(input=txtnet['hash'], kernel=[1, 1, bit, numclass], strides=[1, 1, 1, 1], padding='VALID',
#                                             init_rate=1.0, name='txt_label'), name='txt_label_BN'))
#
#         # return tf.squeeze(txtnet['hash']), txtnet['feature'], tf.squeeze(txtnet['label'])
#         return tf.squeeze(txtnet['hash']), feature['sqzd_feature'], tf.squeeze(txtnet['label'])
...
def lab_net(input_label, bit, numClass, reuse=False, name='Lab_Network'):
    with tf.variable_scope(name):
        if reuse:
            ...
            assert tf.get_variable_scope().reuse is False
            # input_label = tf.expand_dims(tf.expand_dims(input_label, 1), -1)
        ...
        #
        # labnet['conv2'] = lrelu(batch_norm(
        #     conv2d(input=labnet['conv1'], kernel=[1, 1, LAYER1_NODE, LAYER1_NODE], strides=[1, 1, 1, 1],
        #            padding='VALID',
        #            init_rate=1.0, name='lab_ext_conv2'), name='lab_conv2_BN'))
        #
        # labnet['concat'] = tf.concat([labnet['conv1'], labnet['conv2']], axis=2, name="concat")
        #
        # labnet['feature'] = lrelu(
        #     batch_norm(conv2d(input=labnet['concat'], kernel=[1, 2, LAYER1_NODE, SEMANTIC_EMBED], strides=[1, 1, 1, 1],
        #                       padding='VALID', init_rate=1.0, name='lab_feat'), name='lab_feat_BN'))
        #
        # labnet['hash'] = tanh(
        #     batch_norm(conv2d(input=labnet['feature'], kernel=[1, 1, SEMANTIC_EMBED, bit], strides=[1, 1, 1, 1],
        #                       padding='VALID', init_rate=1.0, name='lab_hash'), name='lab_hash_BN'))
        #
        # labnet['label'] = sigmoid(
        #     batch_norm(conv2d(input=labnet['hash'], kernel=[1, 1, bit, numClass], strides=[1, 1, 1, 1],
        #                       padding='VALID', init_rate=1.0, name='lab_label'), name='lab_label_BN'))

        labnet['conv1'] = lrelu(batch_norm(conv2d(input=input_label, kernel=[1, numClass, 1, LAYER1_NODE], strides=[1, 1, 1, 1],
                               padding='VALID', init_rate=1.0, name='lab_conv1'), name='lab_conv1_BN'))

        # labnet['conv2'] = lrelu(batch_norm(
        #     conv2d(input=labnet['conv1'], kernel=[1, 1, LAYER1_NODE, LAYER1_NODE], strides=[1, 1, 1, 1], padding='VALID',
        #            init_rate=1.0, name='txt_ext_conv2'), name='txt_conv2_BN'))
...
def img_net_itpair(inputs, bit, numclass, reuse=False, name='Img_Network'):
    with tf.variable_scope(name):
        ...
        # imgnet['label'] = ((conv2d(input=imgnet['feature'], kernel=[1, 1, SEMANTIC_EMBED, numclass], strides=[1, 1, 1, 1],
        #                                padding='VALID', init_rate=1.0, name='img_label')))
...
# def GCN_stack(input1, input2, name):
#     with tf.variable_scope(name):
#         concat = tf.concat([input1, input2], axis=-1, name="concat")
#         feature = lrelu(batch_norm(
#             conv2d(input=concat, kernel=[1, 1, 2*SEMANTIC_EMBED, SEMANTIC_EMBED], strides=[1, 1, 1, 1], padding='VALID',
#                    init_rate=1.0, name='gc_conv1'), name='gc_conv1_BN'))
#
#         hash = tanh(batch_norm(
#             conv2d(input=feature, kernel=[1, 1, SEMANTIC_EMBED, bit], strides=[1, 1, 1, 1], padding='VALID',
#                    init_rate=1.0, name='gc_hash'), name='gc_hash_BN'))
#         label = conv2d(input=feature, kernel=[1, 1, SEMANTIC_EMBED, numClass], strides=[1, 1, 1, 1], padding='VALID',
#                    init_rate=1.0, name='gc_label')
#
#         # label = sigmoid(conv2d(input=feature, kernel=[1, 1, SEMANTIC_EMBED, numClass], strides=[1, 1, 1, 1], padding='VALID',
#         #            init_rate=1.0, name='gc_label'))
#         return tf.squeeze(hash), tf.squeeze(label)

def GCN_stack(x, indices, data, name):
    with tf.variable_scope(name):
        fc1 = GraphConvLayer(input_dim=SEMANTIC_EMBED,
        # fc1 = GraphConvLayer(input_dim=2*SEMANTIC_EMBED,
        #                      output_dim=1024,
                             output_dim=512,
                             name="fc1",
        ...
        # fc2 = GraphConvLayer(input_dim=1024,
...
# def GCN_stack(x, indices, data, name):
#     with tf.variable_scope(name):
#         fc1 = lg.GraphConvLayer(input_dim=SEMANTIC_EMBED,
#         # fc1 = GraphConvLayer(input_dim=2*SEMANTIC_EMBED,
#                              output_dim=1024,
#                              # output_dim=512,
#                              name="fc1",
...
#                              x=x, sparse=False)
#
#         fc2 = lg.GraphConvLayer(input_dim=1024,
#         # fc2 = GraphConvLayer(input_dim=512,
#                              output_dim=bit,
#                              name="fc2",
#                              activation=tf.nn.tanh,
...
#
#
#         # psu_lab = lg.GraphConvLayer(input_dim=1024, # 1024
#         # # psu_lab = GraphConvLayer(input_dim=512,
#         #                      output_dim=dimLab,
#         #                      name="psu_lab",
...
def full_conv_stack(x, adj_norm):
    with tf.variable_scope("graph_Layer"):
        # weight_1 = tf.get_variable(name='weight_1', shape=[1, 1, SEMANTIC_EMBED, 1024],
        ...
        # weight_3 = tf.get_variable(name='weight_3', shape=[1, 1, 1024, dimLab],
        ...
        # fc_2 = full_conv_no_bias(input=fc_1,
        #                          weights=weight_2,
        #                          adj=adj_norm,
        #                          activation=tf.nn.tanh)
...
def sum_to_vec_1(x):
    with tf.variable_scope("mlp_1"):
        ...
        vec_1 = lrelu(batch_norm(
            # tf.nn.conv2d(tf.transpose(x, [0, 1, 3, 2]), weight_1, strides=[1, 1, 1, 1], padding='VALID', name='vec_1'), name='vec_1_BN'))
