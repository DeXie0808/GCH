...
class GH(object):
    ...
    def _build_model(self):
        ...
        self.Loss_I_lab = self.mse_loss(self.Hsh_I, self.ph['B']) \
                        + self.mse_loss(self.Lab_I, self.ph['L_batch'])
        ...
        self.Loss_I_hsh = self.Loss_pair_i + self.mse_loss(self.Hsh_I, self.ph['B'])
    ...
    def Train(self):
        ...
        for epoch in range(Epoch):
            ...
            if epoch % 1 == 0:
                ...
                if epoch <= MAX_ITER:
                    ...
                    for idx in range(5):
                        ...
                        self.train_txt(learn_rate, update='label')
                    ...
                    for idx in range(5):
                        ...
                        self.train_img(learn_rate, update='label')
            ...
            #             # Train
            #             learn_rate = self.sess.run(self.lr_txt, feed_dict={self.ph['epoch']: epoch})
            #             self.train_txt(learn_rate, update='graph')
            #
            #         for idx in range(5):
            #             # Train
            #             learn_rate = self.sess.run(self.lr_img, feed_dict={self.ph['epoch']: epoch})
            #             self.train_img(learn_rate, update='graph')
            #
            # print "********test************"
            # self.test(self.phase)
            # #
            # # if np.mod(epoch, self.save_freq) == 0:
